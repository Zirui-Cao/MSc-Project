...
#
# # train_ds = tf.convert_to_tensor(x_train)
# # val_ds = tf.convert_to_tensor(x_val)
# # test_ds = tf.convert_to_tensor(x_test)
#
# # import tensorflow_datasets as tfds
# # tf.compat.v1.enable_eager_execution()
# # x_train = tfds.load("voc", split=tfds.Split.TRAIN)
# # x_val = tfds.load("voc", split=tfds.Split.TEST)
# # x_test = tfds.load("voc", split=tfds.Split.VALIDATION)
# # train_ds = tf.data.Dataset.from_tensor_slices(x_train)
# # train_ds = train_ds.shuffle(BUFFER_SIZE).batch(BATCH_SIZE).prefetch(AUTO)
# # val_ds = tf.data.Dataset.from_tensor_slices(x_val)
# # val_ds = val_ds.batch(BATCH_SIZE).prefetch(AUTO)
