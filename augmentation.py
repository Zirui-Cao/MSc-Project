...
def get_random_batch_centre(image_tensor, crop_size, depth_crop_size, pad=20):
    ...
    drc = tf.clip_by_value(drc, tf.cast(depth_crop_size + pad, tf.float32),
                           tf.cast(tf.shape(image_tensor)[1] - depth_crop_size - pad, tf.float32))
    hrc = tf.clip_by_value(hrc, tf.cast(crop_size + pad, tf.float32),
                           tf.cast(tf.shape(image_tensor)[2] - crop_size - pad, tf.float32))
    wrc = tf.clip_by_value(wrc, tf.cast(crop_size + pad, tf.float32),
                           tf.cast(tf.shape(image_tensor)[3] - crop_size - pad, tf.float32))
...
def apply_valid_random_crop_3d(image_tensor,
    ...
    def crop_per_batch(x, y, centre, crop_size, depth_crop_size, resize, output_slice):
        if resize:
            ...
            new_height = tf.cast(original_height + tf.random.uniform([], minval=-original_height * factor,
                                                                     maxval=original_height * factor), tf.int32)
            new_width = tf.cast(
                original_width + tf.random.uniform([], minval=-original_width * factor, maxval=original_width * factor),
                tf.int32)
        ...
        dc, hc, wc = centre
        if output_slice:
            x = tf.slice(x, [dc - depth_crop_size, hc - crop_size, wc - crop_size, 0],
                         [1 + (depth_crop_size * 2), crop_size * 2, crop_size * 2, -1])
            y = tf.slice(y, [dc, hc - crop_size, wc - crop_size, 0], [1, crop_size * 2, crop_size * 2, -1])
        else:
            x = tf.slice(x, [dc - depth_crop_size, hc - crop_size, wc - crop_size, 0],
                         [depth_crop_size * 2, crop_size * 2, crop_size * 2, -1])
            y = tf.slice(y, [dc - depth_crop_size, hc - crop_size, wc - crop_size, 0],
                         [depth_crop_size * 2, crop_size * 2, crop_size * 2, -1])
        return x, y, centre

    if random_shift:
        centre = get_random_batch_centre(image_tensor, crop_size, depth_crop_size)
        image_tensor, label_tensor, centre = tf.map_fn(
            lambda x: crop_per_batch(x[0], x[1], x[2], crop_size, depth_crop_size, resize, output_slice),
            (image_tensor, label_tensor, centre))
    else:
...
def crop_3d(img, crop_size, depth_crop_size, centre, output_slice, target=None):
    ...
    if output_slice:
        if target:
            ...
            img = tf.slice(img, [0, dc - depth_crop_size, hc - crop_size, wc - crop_size, 0],
                           [-1, 1 + (depth_crop_size * 2), crop_size * 2, crop_size * 2, -1])
    else:
        img = tf.slice(img, [0, dc - depth_crop_size, hc - crop_size, wc - crop_size, 0],
                       [-1, depth_crop_size * 2, crop_size * 2, crop_size * 2, -1])
    return img
...
def apply_random_gamma_3d(image_tensor, label_tensor):
    ...
    image_tensor = tf.cond(do_gamma, lambda: tf.image.adjust_gamma(image_tensor, gamma=gamma, gain=gain),
                           lambda: image_tensor)
    return image_tensor, label_tensor
...
def apply_flip_3d(image_tensor, label_tensor):
    image_tensor, label_tensor = tf.map_fn(lambda x: apply_flip_3d_axis(x[0], x[1], axis=-2),
                                           (image_tensor, label_tensor))
    image_tensor, label_tensor = tf.map_fn(lambda x: apply_flip_3d_axis(x[0], x[1], axis=-3),
                                           (image_tensor, label_tensor))
    image_tensor, label_tensor = tf.map_fn(lambda x: apply_flip_3d_axis(x[0], x[1], axis=-4),
                                           (image_tensor, label_tensor))
    return image_tensor, label_tensor
...
def apply_valid_random_crop_3d(image_tensor, label_tensor, crop_size, depth_crop_size, resize, random_shift,
                               output_slice, factor=0.04):
    def crop_per_batch(x, y, centre, crop_size, depth_crop_size, resize, output_slice):
        if resize:
            original_height = tf.cast(tf.shape(x)[1], tf.float32)
            original_width = tf.cast(tf.shape(x)[2], tf.float32)
